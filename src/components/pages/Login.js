        ...
        if (!formValid) {
                    ...
                    <div className="card shadow-sm border-0 px-3 text-align-center rounded-2 mb-3 py-4 mx-auto mt-5 bg-light">
                        ...
                        <div className="card-body">
                            <form action="/" onSubmit={(e) => this.handleSubmit(e)} encType="multipart/form-data" autoComplete="off">
                                ...
                                <br />
                                <div className="form-group">
                                    <label className="mb-0">Password (required)</label>
                                    <input name="contact" type="phone" className="form-control" placeholder="password" value={this.state.phone} onChange={this.handleChange} value={this.state.contact} />
                                </div>
                                <div className="form-group">
                                    <label className="mb-0">Retype-Password (required)</label>
                                    <input name="contact" type="phone" className="form-control" placeholder="password" value={this.state.phone} onChange={this.handleChange} value={this.state.contact} />
                                </div>
                                <br />
                                <div className="form-group">
                                    <label className="mb-0"><span className="text-danger"></span></label>
                                    <br />
                                    <textarea name="message" type="text" className="form-control" placeholder="any special message?" value={this.state.message} onChange={this.handleChange} onBlur={this.handleBlur} />
                                    {messageError
            ...
            return (
                <div className="thankyou_details">
                    <div className="alert alert-success mt-3">Thank for your Register. You May Now Register</div>
                    <ul className="list-group">
                        <li className="list-group-item">Name: {this.state.name}</li>
                        <li className="list-group-item">Email: {this.state.email}</li>
                        <li className="list-group-item" display="hidden">password: {this.state.password}</li>
                        <li className="list-group-item">Message: {this.state.message}</li>
                        {/* <li className="list-group-item">password: {this.state.message}</li>
                        <li className="list-group-item">password: {this.state.message}</li> */}
                    </ul>
                </div>
            )
